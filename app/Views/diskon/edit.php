    ...
    <div class="col-lg-6">
        ...
        <div class="card my-3">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Ubah Diskon</h6>
            </div>
            <div class="card-body">
                <form action="/diskon/update" method="post">
                    <?= csrf_field(); ?>
                    <div class="form-group row">
                        <label for="nama_diskon" class="col-sm-3 col-form-label">Nama Diskon</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="nama_diskon" id="nama_diskon" value="<?= $diskon['nama_diskon']; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="kode_akun_diskon" id="kode_akun_diskon" class="form-control custom-select">
                                ...
                                <?php foreach ($akun as $akun_diskon) : ?>
                                    <option value="<?= $akun_diskon['id']; ?>" <?= ($akun_diskon['id'] == $diskon['akun_diskon_id'] ? 'selected' : ''); ?>><?= $akun_diskon['kode_kategori_akun']; ?>-<?= $akun_diskon['kode_akun']; ?> - <?= $akun_diskon['nama_akun']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="kode_diskon" id="kode_diskon" value="<?= $diskon['kode_diskon']; ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="deskripsi_diskon" class="col-sm-3 col-form-label">Deskripsi Diskon</label>
                        <div class="input-group col-sm-9">
                            <textarea name="deskripsi_diskon" rows="2" class="form-control" id="deskripsi_diskon"><?= $diskon['deskripsi_diskon']; ?></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="jumlah_diskon" class="col-sm-3 col-form-label">Jumlah Diskon</label>
                        <div class="input-group col-sm-9">
                            <input type="number" class="form-control col-9" name="jumlah_diskon" id="jumlah_diskon" value="<?= $diskon['jumlah_diskon']; ?>">
                            <select name="satuan_diskon" id="satuan_diskon" class="form-control custom-select col-3">
                                <option>Satuan ...</option>
                                <option value="persen" <?= ($diskon['satuan_diskon']) == 'persen' ? 'selected' : ''; ?>>Persen (%)</option>
                                <option value="jumlah" <?= ($diskon['satuan_diskon']) == 'jumlah' ? 'selected' : ''; ?>>Rupiah (Rp.)</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="input-group col-sm-9">
                            <input type="text" class="form-control" name="periode_awal_diskon" id="periode_awal_diskon" value="<?= $diskon['periode_awal_diskon']; ?>">
                            <label class="text px-1 col-form-label">s.d.</label>
                            <input type="text" class="form-control" name="periode_akhir_diskon" id="periode_akhir_diskon" value="<?= $diskon['periode_akhir_diskon']; ?>">
                        </div>
                    </div>
