...
<div class="row">
    <div class="col-lg-6">
        ...
        <div class="card my-3">
            ...
            <div class="card-body">
                <form action="/produk/insertcategory" method="post">
                    <?= csrf_field(); ?>
                    <div class="form-group row">
                        <label for="nama_kategori_produk" class="col-sm-3 col-form-label">Nama Kategori Produk</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="nama_kategori_produk" id="nama_kategori_produk" value="<?= old('nama_kategori_produk'); ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="kode_kategori_produk" class="col-sm-3 col-form-label">Kode Kategori Produk</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="kode_kategori_produk" id="kode_kategori_produk" value="<?= old('kode_kategori_produk'); ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="kode_akun_produk" id="kode_akun_produk" class="form-control custom-select">
                                ...
                                <?php foreach ($akun as $akun_produk) : ?>
                                    <option value="<?= $akun_produk['id']; ?>" <?= ($akun_produk['id'] == old('kode_akun_produk') ? 'selected' : ''); ?>><?= $akun_produk['kode_kategori_akun']; ?>-<?= $akun_produk['kode_akun']; ?> - <?= $akun_produk['nama_akun']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
