    ...
    <div class="col-lg-6">
        ...
        <div class="card my-3">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Edit Data Kas Keluar</h6>
            </div>
            <div class="card-body">
                <form action="/kas/update" method="post">
                        ...
                        <div class="col-sm-9">
                            <select name="kode_akun_kas" id="kode_akun_kas" class="form-control custom-select">
                                ...
                                <?php foreach ($akun as $akun_kas) : ?>
                                    <option value="<?= $akun_kas['id']; ?>" <?= ($akun_kas['id'] == $kas_keluar['akun_id'] ? 'selected' : ''); ?>><?= $akun_kas['kode_kategori_akun']; ?>-<?= $akun_kas['kode_akun']; ?> - <?= $akun_kas['nama_akun']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select name="jenis_pajak" id="jenis_pajak" class="form-control custom-select">
                                ...
                                <?php foreach ($pajak as $pajak) : ?>
                                    <option value="<?= $pajak['id']; ?>" <?= ($pajak['id'] == $kas_keluar['pajak_id'] ? 'selected' : ''); ?>><?= $pajak['jenis_pajak']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="input-group col-sm-9">
                            <textarea name="deskripsi_kas" rows="2" class="form-control" id="deskripsi_kas"><?= $kas_keluar['deskripsi']; ?></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="input-group col-sm-9">
                            ...
                            <input type="text" class="form-control" name="jumlah_kas" id="jumlah_kas" value="<?= $kas_keluar['jumlah']; ?>">
                        </div>
                    </div>
                    <input type="hidden" name="kas_keluar_id" value="<?= $kas_keluar['id']; ?>">
                    <input type="hidden" name="pengeluaran_id" value="<?= $kas_keluar['pengeluaran_id']; ?>">
                    <input type="hidden" name="jenis_kas" value="keluar">
